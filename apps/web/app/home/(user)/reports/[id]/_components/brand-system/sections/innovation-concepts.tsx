    ...
    return (
      <Section id="innovation-concepts" className="mt-20">
        ...
        <ArticleBlock className="mt-10">
          {data.recommended_innovation && (
            <RecommendedInnovationCard innovation={data.recommended_innovation} />
          )}
            ...
            <ContentBlock withBorder>
              ...
              <div className="mt-6 space-y-8">
                {data.parallel_investigations.map((inv, idx) => (
                  <ParallelInvestigationCard key={inv.id || idx} investigation={inv} />
                ))}
              </div>
            </ContentBlock>
          )}
...
const RecommendedInnovationCard = memo(function RecommendedInnovationCard({
  ...
  return (
    <div className="border-l-4 border-zinc-900 pl-6">
      ...
      <h3 className="mt-4 text-[24px] font-semibold leading-[1.2] tracking-[-0.02em] text-zinc-900">
        {innovation.title}
      </h3>

      {(innovation.score || innovation.confidence) && (
        <div className="mt-2 flex items-center gap-4 text-[14px] text-zinc-500">
          {innovation.score && <span>Score: <span className="font-medium text-zinc-700">{innovation.score}</span></span>}
          {innovation.confidence && <span>Confidence: <span className="font-medium text-zinc-700">{innovation.confidence}%</span></span>}
        </div>
      )}
      ...
      {innovation.selection_rationale && (
        <div className="mt-6 border-l-2 border-zinc-200 pl-4">
          ...
          {innovation.selection_rationale.why_this_one && (
            <BodyText className="mt-2">{innovation.selection_rationale.why_this_one}</BodyText>
          )}
          {innovation.selection_rationale.ceiling_if_works && (
            <p className="mt-2 text-[16px] leading-[1.3] text-zinc-600">
              <span className="font-medium text-zinc-700">If it works:</span> {innovation.selection_rationale.ceiling_if_works}
            </p>
          )}
        </div>
      )}
        ...
        <div className="mt-6">
          <AccentBorder>
            ...
            {innovation.the_insight.what && (
              <p className="mt-2 text-[18px] font-medium leading-[1.3] tracking-[-0.02em] text-zinc-900">
                {innovation.the_insight.what}
              </p>
            )}
          </AccentBorder>
        ...
        <div className="mt-6 bg-zinc-50 p-4">
          ...
          <div className="mt-3 space-y-2 text-[16px]">
            {innovation.breakthrough_potential.if_it_works && (
              <p className="text-zinc-700"><span className="font-medium">If it works:</span> {innovation.breakthrough_potential.if_it_works}</p>
            )}
            {innovation.breakthrough_potential.estimated_improvement && (
              <p className="text-zinc-700"><span className="font-medium">Improvement:</span> {innovation.breakthrough_potential.estimated_improvement}</p>
            )}
          </div>
        </div>
      )}
      ...
      {innovation.validation_path && (
        <div className="mt-6 border-t border-zinc-200 pt-6">
          ...
          <MetadataGrid
            className="mt-3"
            items={[
              { label: 'Gating Question', value: innovation.validation_path.gating_question },
              { label: 'First Test', value: innovation.validation_path.first_test },
    ...
    <div className="border-l-2 border-zinc-200 pl-6">
      ...
      {(investigation.score || investigation.confidence) && (
        <div className="mt-1 flex items-center gap-3 text-[13px] text-zinc-500">
          ...
          {investigation.confidence && <span>Confidence: {investigation.confidence}%</span>}
        </div>
      )}

      {investigation.one_liner && (
        <p className="mt-2 text-[16px] leading-[1.3] text-zinc-600">{investigation.one_liner}</p>
      )}

      {investigation.what_it_is && (
        <p className="mt-3 text-[16px] leading-[1.3] text-[#1e1e1e]">{investigation.what_it_is}</p>
      )}

      <div className="mt-3 space-y-1 text-[14px]">
        {investigation.ceiling && (
          <p className="text-zinc-600"><span className="font-medium text-zinc-700">Ceiling:</span> {investigation.ceiling}</p>
        )}
        {investigation.key_uncertainty && (
          <p className="text-zinc-600"><span className="font-medium text-zinc-700">Key uncertainty:</span> {investigation.key_uncertainty}</p>
        )}
        {investigation.when_to_elevate && (
          <p className="text-zinc-500"><span className="font-medium">Elevate when:</span> {investigation.when_to_elevate}</p>
        )}
      </div>
    </div>
  );
