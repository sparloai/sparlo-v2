    ...
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl shadow-zinc-200/50">
      ...
      >
        ...
        <aside className="hidden overflow-y-auto border-r border-zinc-100 bg-zinc-50/50 p-6 md:block">
          {/* Navigation */}
          <nav>
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.1em] text-zinc-400">
              Contents
            </p>
          ...
          <div className="mt-8 rounded-xl border border-zinc-200 bg-white p-4">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.1em] text-zinc-400">
              Report Details
            </p>
            <div className="space-y-3 text-[13px]">
              <div className="flex justify-between">
                <span className="text-zinc-500">Read time</span>
                <span className="font-medium text-zinc-900">{report.readTime}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-500">Patents cited</span>
                <span className="font-medium text-zinc-900">{report.patents}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-500">Papers reviewed</span>
                <span className="font-medium text-zinc-900">{report.papers}</span>
              </div>
            </div>
          </div>
        </aside>
            ...
            <div className="flex gap-1 px-4 py-2">
              {reportSections.map((section) => (
                <button
                  key={section.id}
                  onClick={() => scrollToSection(section.id)}
                  className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-[13px] transition-all ${
                    activeSection === section.id
                      ? 'bg-zinc-900 font-medium text-white'
