...
export default function HybridNewReportPage() {
  ...
  useEffect(() => {
    ...
    if (prefill || errorType) {
      window.history.replaceState(
        {},
        '',
        getAppPath('/app/reports/hybrid/new'),
      );
    }
  }, [searchParams]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = await startHybridReportGeneration({
      ...
      console.error('Failed to start hybrid report:', err);
      setFormState((prev) => ({
        ...prev,
        error:
          err instanceof Error
            ? err.message
            : 'Failed to start hybrid report generation',
        isSubmitting: false,
      }));
    }
  ...
  // Input phase - Hybrid Mode design with amber/orange accents
  return (
    <div
    ...
    >
      {/* Ambient Background Glows - Hybrid mode uses amber/orange */}
      <div className="pointer-events-none absolute top-1/4 left-1/4 h-[500px] w-[500px] rounded-full bg-amber-900/10 opacity-0 blur-[120px] dark:opacity-50" />
      <div className="pointer-events-none absolute right-1/4 bottom-1/4 h-[400px] w-[400px] rounded-full bg-orange-900/5 opacity-0 blur-[100px] dark:opacity-50" />
          ...
          <div className="relative flex flex-col overflow-hidden rounded-2xl bg-[--surface-elevated] shadow-lg dark:shadow-2xl dark:shadow-black/50">
            ...
            <div className="flex items-center justify-between border-b border-[--border-subtle] bg-[--surface-overlay] px-6 py-3 dark:bg-neutral-900/20">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <Paperclip className="h-3 w-3 transition-colors group-hover/btn:text-amber-500" />
                </button>
              </div>
            </div>

            {/* Hybrid Mode Info Banner */}
            <div className="border-b border-amber-500/20 bg-amber-500/5 px-6 py-3">
              <p className="text-xs text-amber-700 dark:text-amber-300">
                <strong>Hybrid Mode</strong> searches the full solution spectrum
                from simple to paradigm-shifting. Evaluates on MERIT - the best
                solution wins regardless of origin. Includes honest
                self-critique.
              </p>
            </div>
            ...
            <div className="flex min-h-[320px] flex-col p-6 md:p-8">
              <textarea
                ...
                autoFocus
                data-test="hybrid-challenge-input"
                placeholder="Describe the challenge. Hybrid Mode will search all approaches from simple to revolutionary..."
                spellCheck={false}
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isDetected
                          ? 'border border-amber-500/30 bg-amber-500/10 dark:border-amber-500/20'
              ...
              <div className="flex flex-col items-center justify-between gap-4 px-4 py-2 md:flex-row">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    data-test="hybrid-challenge-submit"
                    className={cn(
                      ...
                      canSubmit && !isSubmitting
                        ? 'bg-amber-600 text-white shadow-lg shadow-amber-500/25 hover:bg-amber-500 hover:shadow-amber-500/40 dark:bg-amber-600 dark:shadow-amber-500/20 dark:hover:bg-amber-500'
                  ...
                  >
                    <span
                      ...
                      style={{ fontFamily: "'Suisse Intl', Inter, sans-serif" }}
                    >
                      {isSubmitting ? 'Analyzing...' : 'Run Hybrid Analysis'}
                    </span>
                    {!isSubmitting && (
